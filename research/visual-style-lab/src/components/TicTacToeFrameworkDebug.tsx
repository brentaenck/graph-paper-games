...
const TicTacToeFrameworkDebug: React.FC = () => {
  ...
  // Handle cell click
  const handleCellClick = useCallback((position: GridCoordinate) => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <div className="ui-text-sm space-y-1">
                ...
                <div>Current Player: {gameState.currentPlayer} ({gameState.players[gameState.currentPlayer]?.name})</div>
                <div>Moves Made: {gameState.moves.length}</div>
                <div>Board State Available: {metadata?.boardState ? 'Yes' : 'No'}</div>
                {metadata?.boardState && (
                  <div>Board: {JSON.stringify(metadata.boardState)}</div>
                )}
              </div>
            </div>
            <div>
