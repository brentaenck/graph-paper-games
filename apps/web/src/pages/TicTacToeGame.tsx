...
const TicTacToeGame: React.FC = () => {
  ...
  const handleAIMove = useCallback(async (currentState: GameState) => {
    ...
    try {
      ...
      const aiMoveResult = await ai.getMove(
        currentState, 
            ...
            <div className="game-over">
              {gameStats.winner ? (
                <div className="winner-announcement">
                  🎉 <strong>{gameStats.winner === 'player1' ? gameConfig.player1Name : gameConfig.player2Name}</strong> wins!
                </div>
              ) : (
            ...
            <div className="current-turn">
              {isThinking ? (
                ...
                <div className="player-turn">
                  <span className={`player-symbol symbol-${gameState.currentPlayer === 0 ? 'x' : 'o'}`}>
          ...
          <div className="tic-tac-toe-board">
            {metadata.boardState.map((row: (TicTacToeSymbol | null)[], rowIndex: number) => (
              row.map((cell: TicTacToeSymbol | null, colIndex: number) => (
                ...
                >
                  {cell && (
                    <span className={`symbol symbol-${cell.toLowerCase()}`}>
                      {cell}
                    </span>
                  )}
                </button>
        ...
        <div className="game-info">
          <div className="game-stats">
            ...
            <div className="stat">
              ...
              <span className="stat-value">{Math.floor((Date.now() - gameStats.gameStartTime) / 1000)}s</span>
            </div>
        ...
        <div className="player-info">
          ...
          <div className={`player-card ${gameState.currentPlayer === 1 ? 'active' : ''}`}>
            ...
            <div className="player-details">
              ...
              <span>{gameConfig.gameMode === 'human-vs-ai' ? `AI Level ${gameConfig.aiDifficulty}` : 'Player 2'}</span>
            </div>
          </div>
        </div>
      </div>
