...
import { err, ok } from '@gpg/shared';
import type { DotsAndBoxesMove, DotsAndBoxesMetadata, MoveEvaluation, AIHint } from './types';
...
export class DotsAndBoxesAI {
  ...
  private async getEasyMove(
    gameState: GameState,
    moves: DotsAndBoxesMove[]
  ): Promise<DotsAndBoxesMove> {
  ...
  private evaluateMove(gameState: GameState, move: DotsAndBoxesMove): MoveEvaluation {
    ...
    const newMetadata = newState.metadata as unknown as DotsAndBoxesMetadata;
  ...
  private calculateStrategicValue(
    ...
    const distance = Math.sqrt(
      Math.pow(move.position.col - centerX, 2) + Math.pow(move.position.row - centerY, 2)
    );
  ...
  private calculateEndgameValue(gameState: GameState, move: DotsAndBoxesMove): number {
    ...
    const metadata = moveResult.data.metadata as unknown as DotsAndBoxesMetadata;
