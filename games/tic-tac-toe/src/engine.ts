...
export class TicTacToeEngine implements GameEngineAPI {
  ...
  createInitialState(settings: GameSettings, players: readonly Player[]): Result<GameState> {
    try {
      ...
      if (settings.gameType !== 'tic-tac-toe') {
        return err(
          'INVALID_GAME_STATE',
          'Invalid game type for Tic-Tac-Toe engine',
      ...
      return err(
        'INVALID_GAME_STATE',
        'Failed to create initial game state',
        { error: String(error) }
      );
    }
  }
  ...
  applyMove(state: GameState, move: Move): Result<GameState> {
    try {
      ...
      const newGrid: Grid = {
        ...
        cells: newBoard.map((row, y) =>
          row.map((cell, x) => ({
            ...
            metadata: winResult.winningLine && 
              isPartOfWinningLine({ x, y }, winResult.winningLine) 
              ? { isWinningCell: true } 
              : {}
          } as GridCell))
        )
      ...
      const nextPlayerIndex = winResult.winner || isGameDraw 
      ...
      return err(
        ...
        { error: String(error), move: move.id }
      );
    }
  }
  ...
  getLegalMoves(state: GameState, playerId: string): readonly Move[] {
    ...
    return emptyPositions.map(position => 
      createMove(position, symbol, playerId)
    );
  }
      ...
      const grid: Grid = {
        ...
        cells: metadata.boardState.map((row, y) =>
          row.map((cell, x) => ({
            ...
            state: cell ? 'occupied' : 'empty',
            owner: cell || undefined,
            metadata: metadata.winningLine && 
              isPartOfWinningLine({ x, y }, metadata.winningLine) 
              ? { isWinningCell: true } 
              : {}
          } as GridCell))
        )
