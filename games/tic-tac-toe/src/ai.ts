...
import type { GameAI, GameState, Move, Result, Hint, AIDifficulty } from '@gpg/shared';

import { createOk as ok, createErr as err } from '@gpg/shared';

import type { TicTacToeMetadata, TicTacToeSymbol, BoardState } from './types';
    ...
    try {
      ...
      switch (difficulty) {
        case 1:
          ...
          break;
        case 2:
          chosenPosition = await this.getDefensiveMove(
            metadata.boardState,
            symbol,
            effectiveTimeLimit
          );
          break;
        case 3:
          chosenPosition = await this.getBasicStrategyMove(
            metadata.boardState,
            symbol,
            effectiveTimeLimit
          );
          break;
        case 4:
          chosenPosition = await this.getMinimaxMove(
            metadata.boardState,
            symbol,
            3,
            effectiveTimeLimit
