...
describe('GameHUD', () => {
  ...
  describe('Player information display', () => {
    ...
    it('should render actual avatar image when provided', () => {
      const turnInfo = createTestTurnInfo({
        currentPlayer: createTestPlayer('player1', 'Alice', { avatar: 'https://example.com/avatar.png' })
      });
  ...
  describe('Button titles and accessibility', () => {
    it('should have appropriate titles on control buttons', () => {
      ...
      expect(screen.getByRole('button', { name: /undo/i }).getAttribute('title')).toBe('Undo last move');
      expect(screen.getByRole('button', { name: /skip/i }).getAttribute('title')).toBe('Skip current turn');
      expect(screen.getByRole('button', { name: /resign/i }).getAttribute('title')).toBe('Resign from game');
    });
