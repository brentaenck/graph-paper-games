...
export const GridRenderer: React.FC<GridRendererProps> = ({
   ...
   */
  const canvasToGrid = useCallback((canvasX: number, canvasY: number): GridCoordinate | null => {
   ...
   */
  const gridToCanvas = useCallback((coordinate: GridCoordinate): { x: number; y: number } => {
    return {
  ...
  const defaultRenderCell = useCallback((
    ...
    if (cell.owner) {
      ...
      ctx.fillText(
        cell.owner.charAt(0).toUpperCase(),
        x + size / 2,
        y + size / 2
      );
    }
